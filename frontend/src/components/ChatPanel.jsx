...
//             // Regex to capture "load ... model [filename]" or just "load ... model"
//             // Handles: "load the model eBikeChasis.glb", "load model foo", "load eBikeChasis.glb", "show model", etc.
//             const loadModelMatch = lowerCmd.match(/(?:load|show|visualize|display)\s+(?:the\s+)?(?:3d\s+)?(?:model|glb|file)(?:\s+(.+))?/);

//             // Explicit check for "load [filename]" where filename ends in .glb or .step (for direct file loading commands without 'model' keyword)
//             const extensionMatch = lowerCmd.match(/(?:load|show|visualize|display)\s+(.+\.(?:glb|gltf|step|stp))/);

//             if (loadModelMatch || extensionMatch) {
//                 const targetName = extensionMatch ? extensionMatch[1] : (loadModelMatch ? loadModelMatch[1] : null);

//                 if (targetName) {
//                     // removing 'the ' if user typed 'load the model the file.glb' (edge case regex) or clean up
//                     let cleanName = targetName.replace(/^the\s+/, '').trim();

//                     // If no extension provided, default to .glb, unless user specifically meant something else?
//                     // User said "eBikeChasis.glb or any another name".
//                     // If they say "eBikeChasis", we assume eBikeChasis.glb
//                     if (!cleanName.includes('.')) {
//                         cleanName += ".glb";
//                     }

//                     // Construct URL - Assuming standard backend static serving
//                     // We try to load it from the uploads directory
//                     const fileUrl = `http://127.0.0.1:8000/static/uploads/${cleanName}`;

//                     addMessage(`🚀 Loading '${cleanName}' into viewer...`, 'bot');
//                     setLatestResult({ glb_url: fileUrl });
//                     setPanelState('right-maximized');
//                     setLoading(false);
//                     return;

//                 } else {
//                     // No filename specified -> "Load the model" (generic)
//                     // Load the PENDING (most recently converted/uploaded) model
//                     if (pendingModelUrl.current) {
//                         setLatestResult({ glb_url: pendingModelUrl.current });
//                         addMessage('🚀 Loading (recently converted) model into viewer...', 'bot');
//                         setPanelState('right-maximized');
//                     } else if (uploadedFile) {
//                         // Fallback to uploaded file if it was a GLB
//                         // If it was STEP, validation usually happens before, but good fallback
//                         // Note: uploadedFile is just a URL string usually.
//                         addMessage('🚀 Loading uploaded model...', 'bot');
//                         setLatestResult({ glb_url: uploadedFile });
//                         setPanelState('right-maximized');
//                     } else {
//                         addMessage('⚠️ No model is pending to load. Please specify a filename (e.g., "load model chassis.glb") or upload one first.', 'bot');
//                     }
//                     setLoading(false);
//                     return;
//                 }
...
//             const commonKeywords = ['load glb', 'show glb', 'view', 'display', 'rotate', 'zoom', 'transform', 'move', 'scale', 'load the model',"implode the model", 'measure', 'explode', 'color'];
...
const ChatPanel = ({ mode, setMode, onTogglePanel, panelState, setPanelState }) => {
    ...
    const handleSend = async (manualCmd = null, hidden = false) => {
        ...
        if (!manualCmd) {
            setInput('');
            if (attachmentPreview) setAttachmentPreview(null); // Clear UI immediately
            // Reset textarea height manually if needed
            const ta = document.querySelector('.pill-input-field');
            if (ta) ta.style.height = 'auto'; // Reset height
        }
        ...
        if (attachmentPreview) {
            try {
                ...
                // Clear attachment preview handled earlier for responsiveness
                setLoading(false);
            }
        } else {
            // No attachment, just stop loading if it was set
        ...
        if (!attachmentPreview && cmdToSend.trim()) {
            ...
            if (loadModelMatch || extensionMatch) {
                // Priority to loadModelMatch[1] (captured filename after keyword) because extensionMatch is greedy and captures keywords too if they exist.
                const targetName = (loadModelMatch && loadModelMatch[1]) ? loadModelMatch[1] : (extensionMatch ? extensionMatch[1] : null);

                if (targetName) {
                    // removing 'the ' if user typed 'load the model the file.glb' (edge case regex) or clean up
                    let cleanName = targetName.replace(/^the\s+/, '').trim();

                    // If it matches one of our keywords exactly/partially, we might want to ignore it as a "filename" if it's just "model"
                    // But the regex (?:model|glb|file) handles the keyword part.
                    // If targetName is "eBikeChasis.glb", cleanName is "eBikeChasis.glb".

                    // Check if the "name" is actually just a keyword from glb_keywords? 
                    // e.g. "load the model" -> targetName might be null or empty string if regex is good.
                    // If regex group 1 is optional, it is undefined.
            ...
            if (lowerCmd.includes("implode")) {
                useUIStore.getState().setExplodeMode(false);
                addMessage("📉 Imploding model...", 'bot');
                setLoading(false);
                return;
            }
